...
int add_game_object(int tag, int x, int y, int dx, int dy, const void *sprite_data) {
    ...
    set_sprite_slot(index);
    set_sprite_pattern(sprite_data);

    set_sprite_slot(index);
    set_sprite_attributes_ext(sprite[index].slot, sprite[index].x, sprite[index].y, sprite[index].color, 0, true);
...
void reset_game_objects() {
   for (int slot = 0; slot < number_of_sprites; ++slot) {
		...
		set_sprite_slot(sprite[slot].slot);
		set_sprite_attributes_ext(sprite[slot].slot, sprite[slot].x, sprite[slot].y, sprite[slot].color, 0, true);
    }
}
...
void move_player(int x, int y) {
    ...
    set_sprite_slot(sprite[player_slot].slot);
    set_sprite_attributes_ext(sprite[player_slot].slot, sprite[player_slot].x, sprite[player_slot].y, 0, 0, true);
}
...
static bool test_sprites(void) {
	uint8_t test = get_sprite_system_state() && SPRITE_COLLISION_MASK;
	if (!test)
		return false;
